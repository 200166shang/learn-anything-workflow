...
`explanation prepare` formally commits a one-use preparation token bound to the question, profile, learning revision, confirmed source scope, and stable section marker. `commit` accepts only that token and validates the marker, in-scope source versions, evidence classification, explicit semantic review facts, and executable profile checks before publishing a revision. Failed validation leaves the prior revision and locator current. Formal Markdown revisions remain immutable; authoritative locators store the content digest and results-relative logical path, then resolve against the current results root after a workspace move.
