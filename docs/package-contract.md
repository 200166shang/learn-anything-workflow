...
- `source verify` never silently upgrades a pinned reference. It reports `current` or `historical`, availability, the current version, and a per-file change summary for code sources. Git provenance includes the commit and sorted modified/added/deleted paths relative to HEAD, strictly scoped by a literal registered-root pathspec, so a dirty checkout is independently inspectable even when directory names contain glob characters. Renames expand to deleted old plus added new; copies add only the destination (while an independently modified source remains modified). `notes prepare`, `learning module show`, `learning locate`, and `explanation prepare` surface `needs_review`; old note/explanation objects and user corrections remain immutable. A changed key source pauses only questions whose actual explanation evidence depends on it; unrelated module sources do not block that question. Current-code evidence uses exactly `file::symbol` with locator kind `symbol`, or `file:start-end` with kind `line_range`, plus that file's content digest. Bare file locators are rejected, and unchanged files remain reusable only when the current registered inventory and live source availability verify the same digest. Inferences remain explicitly distinct and require review.
