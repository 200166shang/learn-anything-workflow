...
Pass the last observed `expected_revision` on feedback and position changes. A stale change to the same question or position becomes a preserved candidate for the user to resolve; an unrelated question feedback may be safely revalidated and merged by the tool. Never edit learning snapshot files directly.
