...
For a requested Feishu reminder, keep delivery one-way: selection, answers, and teaching remain in Codex. Never infer a recipient, sending identity, or authorization. Use the public `delivery enable/status/tick/reconcile/disable` contract; physical recipient credentials stay in the local environment. An uncertain send is reconciled and never blindly repeated.
