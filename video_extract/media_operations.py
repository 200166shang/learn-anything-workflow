...
def show_operation(config: WorkspaceConfig, operation_id: str) -> dict[str, Any]:
    ...
    if record.get("intent", {}).get("capability_id") == "publish.netease":
        from .netease_publish import show_operation as show_netease_operation
        return show_netease_operation(config, operation_id)
...
def resume_operation(config: WorkspaceConfig, operation_id: str) -> dict[str, Any]:
    ...
    if read_json(path).get("intent", {}).get("capability_id") == "publish.netease":
        from .netease_publish import resume_operation as resume_netease_operation
        return resume_netease_operation(config, operation_id)
...
def reconcile_operation(config: WorkspaceConfig, operation_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        return show_operation(config, operation_id)
    if read_json(path).get("intent", {}).get("capability_id") == "publish.netease":
        from .netease_publish import reconcile_operation as reconcile_netease_operation
        return reconcile_netease_operation(config, operation_id)
    if read_json(path).get("intent", {}).get("capability_id") == "audio.mandarin":
